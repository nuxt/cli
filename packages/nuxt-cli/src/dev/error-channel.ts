...
export function useErrorChannel(options: ErrorChannelOptions = {}): Promise<Channel> {
  if (!channel) {
    const pending = channel = import('my-bad/channel').then(async ({ createChannel }) => createChannel({
      open: true,
      // Opening a file spawns an editor, so confine that to the project, which
      // is not `process.cwd()` when `nuxt dev` was pointed elsewhere.
      root: options.cwd || process.cwd(),
      // The dev server's own `Host` check has already run, and knows what
      // `--host` allowed.
      allowedHosts: true,
      sink: await resolveSink(options),
    }))
    // Caching a rejection would answer every later error with it.
    pending.catch(() => {
      if (channel === pending) {
        channel = undefined
      }
...
const CLI_PROGRESS_SOURCE = 'cli'

/** Progress the app forwarded for work the CLI cannot see. */
const APP_PROGRESS_SOURCE = 'app'
...
export function toBuildProgress(snapshot: ProgressSnapshot): BuildProgress {
  return {
    ...
    source: CLI_PROGRESS_SOURCE,
  }
}

/** Publish the CLI's own progress, resolved against whatever the app is reporting. */
export async function publishCliProgress(snapshot: ProgressSnapshot): Promise<void> {
  const progress = toBuildProgress(snapshot)
  await withErrorChannel(channel => channel.progress(progress))
}
    ...
    void useErrorChannel(options).then((instance) => {
      switch (message.type) {
        ...
        case 'nuxt:dev:error:progress': {
          // Sourced, or the channel would broadcast it verbatim and let it
          // fight the CLI's own sequence for the bar.
          instance.progress({ ...message.progress, source: message.progress.source ?? APP_PROGRESS_SOURCE })
          break
        }
