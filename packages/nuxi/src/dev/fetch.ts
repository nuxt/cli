 ...
 */
function fetchWithNodeHttp(socketPath: string, url: URL, init?: RequestInit & { duplex?: string }): Promise<Response> {
  return new Promise((resolve, reject) => {
    ...
    const req = httpRequest({
      ...
      resolve(new Response(webStream as ReadableStream, {
 ...
 */
export async function fetchAddress(addr: DevAddress, input: string | URL | Request, inputInit?: RequestInit): Promise<Response> {
  let url: URL
  let init: (RequestInit & { duplex?: string }) | undefined

  if (input instanceof Request) {
    url = new FastURL(input.url)
    init = {
      method: input.method,
